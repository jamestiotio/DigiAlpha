...
public class SATSolverSY{
    ...
    public static Environment solve(Formula formula) {
        Environment sln = new Environment();
        return solve(formula.getClauses(), sln);
    }
    ...
    private static Environment solve(ImList<Clause> clauses, Environment env) {
        ...
        if (cSmallest.isUnit()){
            // Check var and put bool.TRUE or bool.FALSE
            env = env.put(l.getVariable(), checkVar(l));
            return solve(substitute(clauses,l), env);
        }else{
            Environment newEnv = solve(substitute(clauses, l), env.put(l.getVariable(), checkVar(l)));
            if (newEnv == null){
                Environment temp = env.put(l.getNegation().getVariable(), checkVar(l.getNegation()));
                return solve(substitute(clauses, l.getNegation()), temp);
            }else{
                return newEnv;
            }
        }
